...
use crate::filesystem::{Error, dump};
use core::arch::{asm, global_asm};
use lemon_shark::{ALLOCATOR, filesystem, logln, println, trap_handler};
...
fn writing_to_file() {
    ...
    let bytes_written =
        write_to_file(other_file.0 as usize, content).expect("Failed to write to file");
...
fn writing_multiple_blocks() {
    use filesystem::api::{create_file, write_to_file};
    use filesystem::{BLOCK_SIZE, Error};
...
fn file_and_directory_with_same_name() {
    use filesystem::api::{create_file, mkdir, write_to_file};
